...
class LocalisationAction(object):
	...
	_result = sri_tiago_navigation.msg.LocalisationResult()
	pub = rospy.Publisher('/mobile_base_controller/cmd_vel', Twist, queue_size=1)

	def __init__(self, name):
		self._action_name = name
		self._as = actionlib.SimpleActionServer(self._action_name, sri_tiago_navigation.msg.LocalisationAction, execute_cb=self.execute_cb, auto_start = False)
		self._as.start()
